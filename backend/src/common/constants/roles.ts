 ...
 *   'admin'  -> may also change board settings
 ...
 * lives in the `board.roles` map: { [userId]: 'admin' | 'member' | 'viewer' }.
...
export const ASSIGNABLE_ROLES: BoardRole[] = [BoardRole.ADMIN, BoardRole.MEMBER, BoardRole.VIEWER];

/** Roles allowed to create/modify/delete board content (cards and tasks) */
export const CONTENT_EDITORS: BoardRole[] = [BoardRole.OWNER, BoardRole.ADMIN, BoardRole.MEMBER];

/** Roles allowed to change board settings */
export const BOARD_MANAGERS: BoardRole[] = [BoardRole.OWNER, BoardRole.ADMIN];
